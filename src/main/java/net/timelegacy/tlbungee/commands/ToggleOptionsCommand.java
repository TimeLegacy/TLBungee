...
public class ToggleOptionsCommand extends Command {
  ...
  public void execute(CommandSender sender, String[] args) {
    ...
    if (args.length < 1) {
      ...
      else optionsList.append("&cPrivateMessages&7, ");

      if (plugin.toggleOptions.get(sender.getName()).isAllowServerLookup())
        optionsList.append("&aServerLookup&7.");
      else optionsList.append("&cServerLookup&7.");

      MessageUtils.sendMessage(
          sender, MessageUtils.MAIN_COLOR + "Options: " + optionsList.toString(), true);
      MessageUtils.sendMessage(
          sender, MessageUtils.ERROR_COLOR + "Toggle these options with /o <name>", true);
      return;
    }
    ...
    if (input.equalsIgnoreCase("PrivateMessages")) {
      plugin
          ...
          .setAllowPrivateMessages(
              !plugin.toggleOptions.get(sender.getName()).isAllowPrivateMessages());
    } else if (input.equalsIgnoreCase("ServerLookup")) {
      plugin
          .toggleOptions
          .get(sender.getName())
          .setAllowServerLookup(!plugin.toggleOptions.get(sender.getName()).isAllowServerLookup());
    } else {
      MessageUtils.sendMessage(
          sender, MessageUtils.ERROR_COLOR + "Invalid option specified.", true);
      return;
    }
    ...
    MessageUtils.sendMessage(
        sender, MessageUtils.MAIN_COLOR + "New Options: " + optionsList.toString(), true);
    MessageUtils.sendMessage(
        sender, MessageUtils.ERROR_COLOR + "Toggle these options with /o <name>", true);
  }
}
